...
const Login = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            // Avval credentials ni set qilamiz, keyin /panel/me chaqiramiz
            setCredentials(username, password);
            const me = await panelApi.getMe();

            // login() ga userData + credentials uzatamiz
            login(
                { username: me.username, name: me.username, role: me.status, id: me.id },
                username,
                password
            );
            ...
            // Xatolikda credentials tozalaymiz
            clearCredentials();
        ...
        <div className="min-h-screen flex items-center justify-center relative overflow-hidden">
            ...
            <div className="absolute inset-0 pointer-events-none">
                <div className="absolute top-[-10%] left-[-5%] w-96 h-96 rounded-full opacity-20"
                     style={{ background: "radial-gradient(circle, hsl(199,89%,48%), transparent 70%)", animation: "float1 8s ease-in-out infinite" }} />
                <div className="absolute bottom-[-10%] right-[-5%] w-80 h-80 rounded-full opacity-15"
                     style={{ background: "radial-gradient(circle, hsl(280,60%,55%), transparent 70%)", animation: "float2 10s ease-in-out infinite" }} />
                <div className="absolute top-[40%] right-[20%] w-64 h-64 rounded-full opacity-10"
                     style={{ background: "radial-gradient(circle, hsl(170,70%,45%), transparent 70%)", animation: "float1 12s ease-in-out infinite reverse" }} />
            </div>
            ...
            <motion.div initial={{ opacity: 0, y: 30, scale: 0.96 }} animate={{ opacity: 1, y: 0, scale: 1 }}
                        transition={{ duration: 0.5, ease: "easeOut" }} className="w-full max-w-md mx-4">
                <div className="glass-strong rounded-3xl p-8 md:p-10">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-5">
                        ...
                        <button type="submit" disabled={loading}
                                ...
                                style={{ background: "linear-gradient(135deg, hsl(199,89%,48%), hsl(280,60%,55%))", color: "hsl(225,25%,8%)", boxShadow: loading ? "none" : "0 4px 20px hsla(199,89%,48%,0.4)" }}>
                            {loading ? (
                                <><div className="h-4 w-4 rounded-full border-2 border-current border-t-transparent animate-spin" />Tekshirilmoqda...</>
                            ) : "Kirish"}
                        </button>
                    </form>
